...
NM_T_VALS = T_VALS(IMPACT_TIMESTEP + 1: end);
%%
size(NM_T_VALS)
[ds_nm, as, a_ts, a_tts, q_ts] = a_solution_t_form(NM_T_VALS, ...
...
for k = IMPACT_TIMESTEP + 1 : length(T_VALS)
    ...
    if k > IMPACT_TIMESTEP
        ...
        %% Loads in normal modes solution
        [ws_nm, w_ts_nm, ps_nm] ...
...
%         xlim([0, max(1e-9, 1.5 * ds_composite(k - IMPACT_TIMESTEP))]);
        ...
        legend(["DNS", "FD analytical (composite)", "FD analytical (outer)", "NM analytical"], "interpreter", "latex");
