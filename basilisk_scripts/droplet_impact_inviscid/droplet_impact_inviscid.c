/* droplet_impact_viscid.c
    ...
    We (hopefully) neglect the viscosity, surface tension and gravity
...
#include "curvature.h" // For the position function
        ...
        if (membrane_output_no % energy_output_freq == 0) {
            ...
            while (in_jet) {
                ...
                for (int k = 0; k <= num_y_points; k++) {
                    ...
                    energy_flux += f_val * (u_x_val - turnover_x_vel) * energy_val;
