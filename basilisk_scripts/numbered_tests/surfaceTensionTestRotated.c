...
double DROP_REFINED_WIDTH = 0.075;
...
double membrane_position(double y) {
/* Continuous function for the membrane position */
    if (y <= MEMBRANE_RADIUS) {
        return mag * (1 - y * y / sq(MEMBRANE_RADIUS));
    } else {
        return 0;
    }
}

double membrane_first_derivative(double y) {
/* Continuous function for the first derivative of the membrane position */
    if (y <= MEMBRANE_RADIUS) {
        return mag * (-2 * y / sq(MEMBRANE_RADIUS));
    } else {
        return 0;
    }
}

double membrane_second_derivative(double y) {
...
array with xy[0] = x and xy[1] = y */
  return sq(xy[0] - membrane_position(xy[1]) - x_drop_centre) + sq(xy[1] - y_drop_centre) \
    - sq(DROP_RADIUS);
}
...
event init (t = 0) {
    ...
    #if AMR
    refine((sq(x - membrane_position(y) - x_drop_centre) + sq(y - y_drop_centre) < sq(DROP_RADIUS + DROP_REFINED_WIDTH)) \
        && (sq(x - membrane_position(y) - x_drop_centre) + sq(y - y_drop_centre)  > sq(DROP_RADIUS - DROP_REFINED_WIDTH)) \
        && (level < MAXLEVEL));
    #endif
...
event accAdjustment(i++) {
    ...
    foreach_face(y) {
        ...
        double Wxf = interpolate(Wy, x, y);
        double Wxxf = interpolate(Wyy, x, y);
